...
def process_video_remix(video_bytes, hflip, speed, mute, blur_pos):
    # 반전 안 함, 1.0배속, 음소거 안 함, 블러 없음이면 원본 그대로 반환 (가공 스킵)
    if not hflip and speed == 1.0 and not mute and blur_pos == "블러 없음":
        return video_bytes
...
# ==========================================
# UI 5. 원클릭 세탁 옵션 바 (요청: 반전 미체크, 블러 없음 기본값)
# ==========================================
with st.expander("⚙️ 영상 세탁 옵션 (필요할 때만 체크하여 사용)", expanded=True):
    op1, op2, op3, op4 = st.columns(4)
    with op1:
        # 요청사항 반영: 좌우 반전 기본 미체크 (value=False)
        opt_flip = st.checkbox("🔄 좌우 반전", value=False)
    with op2:
        opt_spd = st.selectbox("⏩ 배속", [1.0, 1.05, 1.1, 1.15, 1.2], index=2)
    with op3:
        # 요청사항 반영: 블러 없음을 1순위 기본값으로 설정 (index=0)
        opt_blur = st.selectbox(
            "🔲 자막 블러 위치",
            ["블러 없음", "하단 자막 (바닥 20%)", "중하단 자막 (바닥 35% 위)", "중앙 자막 (영상 한가운데)", "상단 자막 (영상 상단 20%)"],
            index=0,
        )
    with op4:
